...
- **Reviewed localization:** `NNWLocalizedString` and the feature-compiled `iOS/` and `Shared/` localization call sites route strings through the configured bundle. `Shared/Localizable.xcstrings` adds exactly 20 reviewed user-visible reset/import, publishing, and highlight entries and otherwise permits only existing `new`-to-`translated` state transitions.
