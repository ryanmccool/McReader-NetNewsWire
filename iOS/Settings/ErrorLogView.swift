...
struct ErrorLogView: View {
	...
	var body: some View {
		VStack(spacing: 0) {
			if entries.isEmpty {
				ContentUnavailableView(NNWLocalizedString("No Errors Logged", comment: "Error log empty state"), systemImage: "checkmark.circle")
	...
	private var helpLinkFooter: some View {
		Button(NNWLocalizedString("Error Log Help", comment: "Help link")) {
			showHelp = true
		}
		.font(.subheadline)
